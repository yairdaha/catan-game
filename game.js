...
function vp(s,pi){
 let n=0;
 Object.values(s.buildings||{}).forEach(b=>{if(b.owner===pi)n+=b.level==='city'?2:1});
 const cards=Array.isArray(s.players?.[pi]?.devCards)?s.players[pi].devCards:Object.values(s.players?.[pi]?.devCards||{});
 n+=cards.filter(c=>VP_TYPES.has(c?.type)).length;
 if(s.longestRoadHolder===pi)n+=2;
 if(s.largestArmyHolder===pi)n+=2;
 return n
}
...
function playable(p){
 const cards=Array.isArray(p?.devCards)?p.devCards:Object.values(p?.devCards||{});
 return cards.filter(c=>!VP_TYPES.has(c?.type)&&Number(c?.boughtTurn||0)<Number(state.turnCounter||0))
}
...
function devSummary(p){
 const cards=Array.isArray(p?.devCards)?p.devCards:Object.values(p?.devCards||{});
 const g={};
 for(const c of cards){
   const n=DEV_NAMES[c?.type]||c?.type||'קלף פיתוח';
   g[n]=(g[n]||0)+1
 }
 return Object.entries(g).map(([n,q])=>`${n} ×${q}`).join(' · ')||'אין'
}
...
${(Array.isArray(me.devCards)?me.devCards:Object.values(me.devCards||{})).map(c=>VP_TYPES.has(c.type)
...
function render(){
 try{
   document.getElementById('app').innerHTML=screen==='home'?home():screen==='lobby'?lobby():game();
   bind();
   const pi=myIndex();
   if(state?.turnPhase==='discard'&&state.pendingDiscards?.[pi]&&!modal){
     modal={type:'discard',need:state.pendingDiscards[pi]};
     renderModal()
   }
 }catch(e){
   console.error('Render error',e);
   const app=document.getElementById('app');
   if(app)app.innerHTML=`<div class="page"><div class="card"><h2>שגיאת תצוגה</h2><p>המשחק עדיין מחובר לחדר. נסה רענון.</p><div class="tiny">${String(e?.message||e)}</div></div></div>`;
 }
}
