...
async function tx(fn){try{return(await runTransaction(roomRef(code),cur=>{if(!cur)return cur;cur.players=ps(cur);normalizeSpectators(cur);cur.buildings=cur.buildings||{};cur.roads=cur.roads||{};return fn(cur)??cur},{applyLocally:false})).snapshot.val()}catch(e){err='הפעולה נכשלה: '+e.message;render()}}
...
async function rescueStuckTurn(){
 ...
 if(!confirm(`לדלג על התור של ${label} ולשחרר את המשחק?`))return;

 await tx(s=>{
   if(s.host!==myId)return s;
   const old=s.currentPlayer;
   const next=nextActivePlayer(s,old);
   if(next===null)return s;
   ...
   s.log=s.log||[];
   s.log.push(`🛠️ המארח שחרר תור תקוע. התור עבר ל-${s.players[next].name}.`);
   return recalc(s)
 })
}
...
</div><div class="sec"><div class="sect">שחקנים</div><div class="plist">${state.players.map((p,i)=>`<div class="pitem"><div class="dot" style="background:${p.color}"></div><div><b style="${p.left?'opacity:.55;text-decoration:line-through':''}">${p.name}</b>${p.left?'<span class="left-badge">יצא</span>':''}<div class="tiny">${p.vp||0} נק' · 🃏 ${total(p)} קלפים · מחבלים: ${p.knightsPlayed||0} · דרך: ${state.longestRoadLengths?.[i]||0}</div></div><div class="player-public-info"><span class="cardcount">${total(p)} 🃏</span><span>${i===state.longestRoadHolder?'🛣️':''}${i===state.largestArmyHolder?'🛡️':''}</span></div></div>`).join('')}</div>${spectators().length?`<div class="sec"><div class="sect">👀 צופים</div><div class="spectator-game-list">${spectators().map(x=>`<span class="spectator-chip">${x.name}${x.id===myId?' (אתה)':''}</span>`).join('')}</div></div>`:''}</div><div class="center"><div class="boardhead"><div><b>הלוח</b><div class="tiny">חממה = יישוב, יישוב = עיר, דרך בין מושבים = כביש</div></div><div class="legend">${RK.map(k=>`<span class="leg">${R[k].icon} ${R[k].name}</span>`).join('')}</div></div><div class="boardwrap">${renderBoard()}</div><div class="hint">${hint()}</div></div><div class="panel right">${offer?`<div class="sec"><div class="sect">עסקה פתוחה</div><div class="offer"><b>${state.players[offer.from].name}</b> נותן ${fmt(offer.give)}<br>ומבקש ${fmt(offer.want)}<div style="height:8px"></div>${watching?'<div class="tiny">👀 צופה אינו יכול להשתתף בעסקה.</div>':offer.from===pi
...
function subscribe(c){if(unsub)unsub();unsub=onValue(roomRef(c),snap=>{const s=snap.val();if(!s){err='החדר לא קיים';screen='home';state=null;render();return}state=s;state.players=ps(state);normalizeSpectators(state);screen=s.phase==='lobby'?'lobby':'game';render();alertMyTurn(state);alertSteal(state)})}
